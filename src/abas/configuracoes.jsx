...
function LockMiniIcon() {
  return (
    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={C.gold} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: "0.5px solid " + C.brd }}>
      ...
      <div style={{ flex: 1, minWidth: 0, cursor: onView ? "pointer" : "default" }} onClick={onView}>
        ...
        <div style={{ fontSize: 11, color: C.tx4 }}>PODER {poder} · {(fp.xp || 0).toLocaleString()} energia</div>
      </div>
...
function DevPanel({ open, onOpen, onClose, profile, setProfile, poderInfo, rankInfo }) {
        ...
        {showPrompt && (
          <div
            style={{
              ...
              background: C.card,
              borderRadius: 8,
              border: "1px solid " + (pwError ? C.red : C.goldBrd),
              transition: "border-color .2s",
            }}
          >
            <span style={{ display: "inline-flex", flexShrink: 0 }}><LockMiniIcon /></span>
            <input
              autoFocus
              type="password"
              value={pwInput}
  ...
  return (
    <div style={{ marginTop: 16, background: C.card, border: "1px solid " + C.goldBrd, borderRadius: 12, padding: 14, marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 11, color: rankInfo.color || C.gold, fontWeight: 600, background: (rankInfo.color || C.gold) + "20", border: "1px solid " + (rankInfo.color || C.gold) + "40", padding: "2px 8px", borderRadius: 5 }}>
      ...
      <div style={{ background: C.goldDim, borderRadius: 8, padding: "8px 10px", marginBottom: 10 }}>
        ...
        <DevField label="Energia total (totalXp)" value={profile.totalXp || 0} min={0} step={100} onChange={v => set("totalXp", v)} />
        <div style={{ fontSize: 10, color: C.gold, opacity: 0.7, marginTop: -4, marginBottom: 4, paddingLeft: 148 }}>PODER {Math.floor((profile.totalXp || 0) / 100)}</div>
        <DevField label="Energia hoje (xpToday)" value={profile.xpToday || 0} min={0} onChange={v => set("xpToday", v)} />
        <DevField label="Melhor dia de energia" value={profile.bestXpDay || 0} min={0} onChange={v => set("bestXpDay", v)} />
      </div>

      <div style={{ background: C.goldDim, borderRadius: 8, padding: "8px 10px", marginBottom: 10 }}>
        <div style={{ fontSize: 10, color: C.gold, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>Moedas</div>
        <DevField label="Moedas" value={profile.coins || 0} min={0} onChange={v => set("coins", v)} />
        <DevField label="Moedas hoje" value={profile.coinsToday || 0} min={0} onChange={v => set("coinsToday", v)} />
        <DevField label="Total de moedas ganhas" value={profile.totalCoinsEarned || 0} min={0} onChange={v => set("totalCoinsEarned", v)} />
      </div>

      <div style={{ background: C.orange + "12", borderRadius: 8, padding: "8px 10px", marginBottom: 10 }}>
        <div style={{ fontSize: 10, color: C.orange, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>Streak</div>
        <DevField label="Streak atual" value={profile.streak || 0} min={0} onChange={v => set("streak", v)} />
        <DevField label="Melhor streak" value={profile.bestStreak || 0} min={0} onChange={v => set("bestStreak", v)} />
      </div>

      <div style={{ background: C.green + "12", borderRadius: 8, padding: "8px 10px", marginBottom: 10 }}>
        <div style={{ fontSize: 10, color: C.green, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>Tarefas / conquistas</div>
        <DevField label="Tarefas concluídas" value={profile.tasksCompleted || 0} min={0} onChange={v => set("tasksCompleted", v)} />
        <DevField label="Projetos concluídos" value={profile.projectsCompleted || 0} min={0} onChange={v => set("projectsCompleted", v)} />
        <DevField label="Maestrias gold" value={profile.masteryGoldCount || 0} min={0} onChange={v => set("masteryGoldCount", v)} />
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 4 }}>
          {[
            ...
            <div
              ...
              style={{ fontSize: 10, cursor: "pointer", padding: "3px 8px", borderRadius: 5, background: profile[key] ? C.green + "30" : C.bg, border: "1px solid " + (profile[key] ? C.green : C.brd), color: profile[key] ? C.green : C.tx4, transition: "all .12s" }}
            >
      ...
      <div style={{ fontSize: 10, color: C.gold, opacity: 0.7, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>Atalhos rápidos de poder</div>
      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
        {[
          ...
          <div key={xp} onClick={() => set("totalXp", xp)} style={{ fontSize: 10, cursor: "pointer", padding: "4px 8px", borderRadius: 5, background: C.goldDim, border: "1px solid " + C.goldBrd, color: C.gold, transition: "background .1s" }}>
            {label}
          </div>
        ))}
      </div>
...
function ConfigTab(props) {
  ...
  const SectionHeader = ({ label, skey }) => (
    <div onClick={() => toggleSection(skey)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", padding: "2px 0", marginBottom: openSections[skey] ? 8 : 0 }}>
      ...
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.tx4} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ transform: openSections[skey] ? "rotate(180deg)" : "rotate(0deg)", transition: "transform .2s", flexShrink: 0 }}>
  ...
  const reset = () => {
    ...
    setProfile(p => ({
      ...
      pendingChest: null,
      streakLostDays: 0,
      purchasedItems: ["t_iniciante", "i_estrela", "obsidiana", "b_simples"],
      equippedTitle: "t_iniciante",
      equippedIcon: "i_estrela",
      equippedTheme: "obsidiana",
      equippedBorder: "b_simples",
      upgradeLevels: {},
      username: p.username,
    }));
    setShowReset(false);
    ...
    <div style={{ padding: 14 }}>
      <div style={{ background: C.card, borderRadius: 14, marginBottom: 14, border: "1px solid " + C.brd, boxShadow: C.goldShadow ? "0 4px 20px " + C.goldShadow : "none", padding: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 10 }}>
          {[
            ...
            [(profile.totalCoinsEarned || 0).toLocaleString(), "Total ganho", C.gold],
          ].map(([v, l, color], i) => (
        ...
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
          {[
            ...
            <div key={i} style={{ fontSize: 11, color: C.tx3, background: C.bg, padding: "3px 8px", borderRadius: 4 }}>
              <span style={{ color: C.tx4 }}>{label}:</span> {name}
            </div>
          ))}
          ...
          {profile.bestXpDay > 0 && <div style={{ fontSize: 11, color: C.tx3, background: C.bg, padding: "3px 8px", borderRadius: 4 }}>Recorde: {profile.bestXpDay} energia/dia</div>}
        </div>
      </div>
      ...
      <div style={{ background: C.card, borderRadius: 10, marginBottom: 14, border: "1px solid " + C.brd, padding: "10px 12px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div onClick={() => setFriendsOpen(o => !o)} style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", flex: 1 }}>
            ...
            {pendingCount > 0 && <span style={{ fontSize: 10, background: C.red, color: "#fff", borderRadius: 8, padding: "1px 5px", fontWeight: 700, lineHeight: "15px" }}>{pendingCount}</span>}
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.tx4} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ transform: friendsOpen ? "rotate(180deg)" : "rotate(0deg)", transition: "transform .2s", marginLeft: 2 }}>
              <polyline points="6 9 12 15 18 9" />
            </svg>
          </div>
          {friendsOpen && profile.username && (
            <span onClick={() => { setShowSearch(true); setSearchQuery(""); setSearchResults([]); }} style={{ fontSize: 11, color: C.gold, cursor: "pointer", fontWeight: 600, marginLeft: 12, flexShrink: 0, display: "flex", alignItems: "center", gap: 3 }}>
              ...
              <>
                <div style={{ display: "flex", borderBottom: "1px solid " + C.brd, marginBottom: 10 }}>
                  {[
                    ...
                    <div key={k} onClick={() => setFriendsTab(k)} style={{ flex: 1, textAlign: "center", padding: "5px 2px 7px", fontSize: 11, cursor: "pointer", fontWeight: friendsTab === k ? 600 : 400, color: friendsTab === k ? C.gold : C.tx4, borderBottom: "2px solid " + (friendsTab === k ? C.gold : "transparent"), transition: "color .12s, border-color .12s", marginBottom: -1 }}>
                      {label}{count > 0 ? " " + count : ""}
                      {k === "recebidos" && count > 0 && <span style={{ marginLeft: 3, width: 6, height: 6, borderRadius: 3, background: C.red, display: "inline-block", verticalAlign: "middle", marginBottom: 1 }} />}
                    </div>
                  ))}
                </div>
                {friendsTab === "amigos" && (
      ...
      <div style={{ background: C.card, borderRadius: 10, marginBottom: 10, border: "1px solid " + C.brd, padding: "12px 14px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
          ...
          <div onClick={() => setShowGroqTutorial(true)} title="Como obter a chave" style={{ width: 20, height: 20, borderRadius: 10, background: C.goldDim, border: "1px solid " + C.goldBrd, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
        ...
        <div style={{ fontSize: 11, color: C.tx3, marginBottom: 8, lineHeight: 1.5 }}>
          Usada para gerar missões e questionários personalizados com IA. Gratuita em <span style={{ color: C.gold }}>console.groq.com</span>.
        </div>
        <div style={{ position: "relative" }}>
          <input
            ...
            placeholder="gsk_..."
            style={{ width: "100%", padding: "8px 10px", background: C.bg, border: "1px solid " + C.brd2, borderRadius: 6, color: C.tx, fontSize: 12, fontFamily: "inherit", outline: "none", boxSizing: "border-box" }}
          />
        </div>
        {profile.groqApiKey && (
      ...
      {showGroqTutorial && (
        <div onClick={() => setShowGroqTutorial(false)} style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.65)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "0 20px" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: C.card, borderRadius: 14, border: "1px solid " + C.brd, padding: "20px 20px 24px", width: "100%", maxWidth: 360, boxShadow: "0 8px 40px rgba(0,0,0,0.5)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              ...
              <div onClick={() => setShowGroqTutorial(false)} style={{ cursor: "pointer", padding: 4, color: C.tx3 }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={C.tx3} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            {[
              { n: "1", text: "Acesse ", highlight: "console.groq.com", after: " no navegador" },
              ...
              { n: "4", text: "Clique em ", highlight: "Create API Key", after: ", dê um nome e confirme" },
              ...
              <div key={i} style={{ display: "flex", gap: 10, marginBottom: i < 4 ? 12 : 0, alignItems: "flex-start" }}>
                <div style={{ width: 22, height: 22, borderRadius: 11, flexShrink: 0, background: C.goldDim, border: "1px solid " + C.goldBrd, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: C.gold }}>{step.n}</div>
            ...
            <div style={{ marginTop: 16, padding: "8px 10px", borderRadius: 8, background: C.bg, border: "1px solid " + C.brd }}>
              <div style={{ fontSize: 11, color: C.tx3, lineHeight: 1.5 }}>O plano gratuito do Groq inclui requisições suficientes para uso diário no app.</div>
            </div>
            <div onClick={() => setShowGroqTutorial(false)} style={{ marginTop: 16, padding: "9px 0", borderRadius: 8, background: C.goldDim, border: "1px solid " + C.goldBrd, textAlign: "center", cursor: "pointer", fontSize: 12, fontWeight: 600, color: C.gold }}>
              Entendido
            </div>
          </div>
        </div>
              ...
              return (
                <div key={cat} style={{ marginBottom: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 11, color: curCat.color, fontWeight: 600 }}>{curCat.id} · {getEnergia(presets[cat])} energia</span>
                  </div>
                  <div style={{ display: "flex", gap: 3 }}>
                    {DIFF_CATEGORIES.map(c => {
                      ...
                      return (
                        <div key={c.id} onClick={() => setPreset(cat, mid)} style={{ flex: 1, paddingTop: 5, paddingBottom: 5, borderRadius: 6, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 1, cursor: "pointer", background: isSel ? c.color + "2e" : c.color + "0e", border: "1px solid " + (isSel ? c.color + "88" : c.color + "28"), transition: "background .1s, border-color .1s" }}>
                          <span style={{ fontSize: 11, fontWeight: 700, color: isSel ? c.color : c.color + "77", lineHeight: 1 }}>{c.id}</span>
                          <span style={{ fontSize: 9, color: isSel ? c.color + "cc" : c.color + "44", lineHeight: 1 }}>{getEnergia(mid)} energia</span>
                        </div>
                      );
                    })}
                  </div>
        ...
        return (
          <div key={item.id + item._type} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", background: C.card, borderRadius: 7, marginBottom: 4, opacity: 0.6 }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: C.tx4 }}>{item._type} · {dl}d restantes{item.autoArchived ? " (autoarquivada)" : ""}</div>
            </div>
      ...
      {usernameEditing && (
        <Modal>
          <div style={{ marginBottom: 14 }}>
            ...
            <input value={usernameInput} onChange={e => { setUsernameInput(e.target.value.toLowerCase().replace(/[^a-z0-9_.]/g, "")); setUsernameError(""); }} placeholder="seunome" maxLength={20} autoFocus style={{ width: "100%", padding: "9px 12px", background: C.bg, border: "1px solid " + (usernameError ? C.red : C.brd), borderRadius: 8, color: C.tx, fontSize: 14, boxSizing: "border-box", outline: "none" }} />
            {usernameError && <div style={{ fontSize: 11, color: C.red, marginTop: 6 }}>{usernameError}</div>}
            {!usernameError && usernameInput.length > 0 && <div style={{ fontSize: 11, color: C.tx4, marginTop: 6 }}>Ficará: <span style={{ color: C.gold, fontWeight: 600 }}>@{usernameInput.trim().toLowerCase().replace(/[^a-z0-9_.]/g, "")}</span></div>}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <Btn onClick={() => { setUsernameEditing(false); setUsernameError(""); }} style={{ flex: 1 }}>Cancelar</Btn>
            <Btn primary onClick={saveUsername} style={{ flex: 1, opacity: usernameSaving ? 0.6 : 1, pointerEvents: usernameSaving ? "none" : "auto" }}>{usernameSaving ? "Verificando..." : "Salvar"}</Btn>
          </div>
        </Modal>
      )}
      ...
      {showSearch && (
        <Modal>
          ...
          <input value={searchQuery} onChange={e => handleSearch(e.target.value)} placeholder="Digite o @username..." autoFocus style={{ width: "100%", padding: "9px 12px", background: C.bg, border: "1px solid " + C.brd, borderRadius: 8, color: C.tx, fontSize: 13, boxSizing: "border-box", outline: "none", marginBottom: 12 }} />
          {searchLoading && <div style={{ fontSize: 11, color: C.tx4, textAlign: "center", padding: "8px 0" }}>Buscando...</div>}
          {!searchLoading && searchQuery.length >= 2 && searchResults.length === 0 && <div style={{ fontSize: 11, color: C.tx4, textAlign: "center", padding: "8px 0" }}>Nenhum usuário encontrado</div>}
          {searchResults.map(r => (
            <div key={r.user_id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: "0.5px solid " + C.brd }}>
              ...
              {r.isAlreadyFriend ? (
                ...
                <span onClick={() => { acceptRequest(friendsData.received.find(f => f.sender_id === r.user_id)?.id); setShowSearch(false); }} style={{ fontSize: 11, color: C.gold, cursor: "pointer", fontWeight: 700, padding: "3px 7px", borderRadius: 4, background: C.goldDim, border: "0.5px solid " + C.goldBrd }}>
                  Aceitar
                </span>
              ) : (
        ...
        return (
          <Modal>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 12, color: fRank.color || C.tx3, marginTop: 3 }}>PODER {fPoder} · {(viewFriend.xp || 0).toLocaleString()} energia</div>
      ...
      {showReset && (
        <Modal>
          <div style={{ textAlign: "center", marginBottom: 14 }}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={C.red} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ display: "block", margin: "0 auto 8px" }}>
            ...
            <input value={resetInput} onChange={e => setResetInput(e.target.value)} placeholder="RESETAR" autoFocus style={{ width: "100%", padding: "7px 10px", background: C.bg, border: "1px solid " + (resetInput === "RESETAR" ? C.red : C.brd), borderRadius: 6, color: C.tx, fontSize: 12, textAlign: "center", boxSizing: "border-box", outline: "none" }} />
          </div>
      ...
      {import.meta.env.DEV && <DevPanel open={devOpen} onOpen={() => setDevOpen(true)} onClose={() => setDevOpen(false)} profile={profile} setProfile={setProfile} poderInfo={_poderInfo} rankInfo={_rankInfo} />}
    </div>
  );
}
